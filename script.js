...
pizzaJson.map((item, index)=>{
  //clonar o modelo do HTML
  let pizzaItem = c('.models .pizza-item').cloneNode(true);
  
  //colocando os itens do array
  ...
  pizzaItem.querySelector('a').addEventListener('click', (e)=>{
    ...
    //colocar a quanridade nos tamanhos:
    cs('.pizzaInfo--size').forEach((size, sizeIndex)=>{
      if(sizeIndex ==2){
        size.classList.add('selected')
      }
      let quantidade = pizzaJson[key].sizes[sizeIndex]
      size.querySelector('span').innerHTML = quantidade
    })

    c('.pizzaInfo--qt').innerHTML = modalQt;

    ///stilizacao da pigina com JS
  ...
  //adicionado item por item no pizza-area
...
c('.pizzaInfo--addButton').addEventListener('click', ()=>{
  ...
  //filtro para add o mesmo item no cart
